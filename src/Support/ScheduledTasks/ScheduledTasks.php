...
class ScheduledTasks
{
    ...
    public function __construct(Schedule $schedule)
    {
        ...
        $this->tasks = collect($this->schedule->events())
            ->filter(
                fn (SchedulerEvent|Event|ScheduledTaskStarting $event): bool => $event->runsInEnvironment(config('app.env'))
            )
            ->map(
                fn (SchedulerEvent|Event|ScheduledTaskStarting $event): Task => ScheduledTaskFactory::createForEvent($event)
            );
    }
